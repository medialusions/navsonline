...
class Auth_Controller extends CI_Controller {
    ...
    private function _redirect_to_login_page() {
        ...
        $redirect = $this->input->get('redirect') ? urlencode($this->input->get('redirect')) : urlencode($this->uri->uri_string());
        ...
        header(
                'Location: ' . site_url(LOGIN_PAGE . '?redirect=' . $redirect, $redirect_protocol), TRUE, 302
        );
    }
    ...
    protected function setup_login_form($optional_login = FALSE) {
        ...
        $redirect = $this->input->get('redirect') ? '?redirect=' . $this->input->get('redirect') : '?redirect=' . config_item('default_login_redirect');
