...
(function ($, window, document) {
    ...
    function PeriodPicker(startinput, options) {
        var that = this, date, tw = new TimeWrapper(), tinputs;
        ...
        that.addRange([
            date.parseStr(that.startinput.val(), !options.timepicker ? options.formatDate : options.formatDateTime) || date.parseStr(that.startinput.val(), options.formatDate),
            date.parseStr(that.endinput.val(), !options.timepicker ? options.formatDate : options.formatDateTime) || date.parseStr(that.endinput.val(), options.formatDate)
        ]);
        ...
        if (options.timepicker && $.fn.TimePicker !== undefined) {
            if (options.useTimepickerLimits) {
                tinputs = that.timepicker.find('input.timepicker');
                if (tw.isEqualDate(that.period[0], that.period[1])) {
                    tinputs.eq(0)
                        .TimePicker('setValue', that.period[0])
                        .TimePicker('setMax', that.period[1]);
                    tinputs.eq(1)
                        .TimePicker('setValue', that.period[1])
                        .TimePicker('setMin', that.period[0]);
                }
            }
